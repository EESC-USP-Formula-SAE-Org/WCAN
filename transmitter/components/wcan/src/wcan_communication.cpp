...
static void SendData(event_send_cb_t send_cb){
...
static void IRAM_ATTR ResendData(TimerHandle_t xTimer) {
    event_send_cb_t *send_cb = (event_send_cb_t*)pvTimerGetTimerID(xTimer);
    if (retry_count < WCAN_RETRY_COUNT) {
        ESP_LOGI("Resend", "Timeout reached, resending %04x... Attempt: %d of %d", send_cb->can_id, retry_count + 1, WCAN_RETRY_COUNT);
        SendData(*send_cb);
        retry_count++;
    } else {
...
static void SendProcessingTask(void *pvParameter)
{
    ...
    while (xQueueReceive(send_queue, &send_cb, portMAX_DELAY) == pdTRUE) {
        ...
        event_send_cb_t *data = (event_send_cb_t*) malloc(sizeof(event_send_cb_t));
        if (data == NULL) {
            ESP_LOGE(TAG, "Malloc send data fail");
            free(send_cb.payload);
            return;
        }
        memcpy(data, &send_cb, sizeof(event_send_cb_t));
        
        ESP_LOGI(TAG, "Send mutex taken (%d)", uxSemaphoreGetCount(send_mutex));
        SendData(*data);
        retry_count = 0;
        //copy resend data
        resend_timer = xTimerCreate("ResendTimer", pdMS_TO_TICKS(ESPNOW_MAXDELAY), pdTRUE, (void *)data, ResendData);
...
static void AckSend(event_recv_cb_t recv_cb)
{
...
static void ESPNOW_RecvCallback(const esp_now_recv_info_t *recv_info, const uint8_t *data, int data_len)
{
    ...
    if (can_id == ack_id) {
        ...
        if (uxSemaphoreGetCount(send_mutex) == 0) {
            retry_count = WCAN_RETRY_COUNT;
            ResendData(resend_timer);
        }
        free(recv_cb->data);
        free(recv_cb);
        return;
