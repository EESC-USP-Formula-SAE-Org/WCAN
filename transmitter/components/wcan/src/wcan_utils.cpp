...
esp_now_packet_t EncodeDataPacket(const data_packet_t data_packet){
    ...
    memcpy(esp_now_packet.data, &data_packet.can_id, sizeof(data_packet.can_id));
    memcpy(esp_now_packet.data + sizeof(data_packet.can_id), data_packet.payload, data_packet.payload_len);
    return esp_now_packet;
}
